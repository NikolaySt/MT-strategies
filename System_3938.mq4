...
string gs_unused_84 = "";
int gi_92 = 3938;
extern string Expert_Name = "Сметка:3938";
extern string Information = "Начален капитал 10`000$ при риск - 1.25%";
extern bool gi_112 = TRUE;
extern bool gi_116 = TRUE;
int gi_120 = 60;
int gi_124 = 1440;
...
double OpenSell(int ai_0, int a_timeframe_4, int ai_8, double ad_12, double ad_20, string a_comment_28, int ai_36, double ad_40 = 0.0, bool ai_48 = TRUE) {
   ...
   int l_ticket_88 = OrderSend(Symbol(), OP_SELL, l_lots_72, Bid, g_slippage_408, l_price_80, ad_40, a_comment_28, 13938, 0, g_color_356);
   if (l_ticket_88 > 0) {
...
double OpenBuy(int ai_0, int a_timeframe_4, int ai_8, double ad_12, double ad_20, string a_comment_28, int ai_36, double ad_40 = 0.0, bool ai_48 = TRUE) {
   ...
   int l_ticket_88 = OrderSend(Symbol(), OP_BUY, l_lots_72, Ask, g_slippage_408, l_price_80, ad_40, a_comment_28, 13938, 0, g_color_348);
   if (l_ticket_88 > 0) {
      if (gi_412) PlaySound(gs_416);
      SuccessOperation();
      return (l_ticket_88);
   ...
   while (!li_20) {
      ...
      for (int l_pos_28 = 0; l_pos_28 < OrdersTotal(); l_pos_28++) {
         if (OrderSelect(l_pos_28, SELECT_BY_POS, MODE_TRADES)) {
            if (OrderSymbol() == Symbol() && OrderMagicNumber() == 13938 && OrderType() == a_cmd_0 && SignalName(OrderComment()) == as_4) {
               if (a_cmd_0 == OP_BUY)
...
void MngSLSave(int a_timeframe_0, string as_4, int ai_12 = 3, int ai_16 = 3, double ad_20 = 1.0, double ad_28 = 0.7) {
   ...
   for (int l_pos_88 = 0; l_pos_88 < OrdersTotal(); l_pos_88++) {
      if (OrderSelect(l_pos_88, SELECT_BY_POS, MODE_TRADES)) {
         if (OrderSymbol() == Symbol() && OrderMagicNumber() == 13938 && SignalName(OrderComment()) == as_4) {
...
int CountOpenAllSignal(string as_0 = "") {
   ...
   for (int l_pos_12 = 0; l_pos_12 < OrdersTotal(); l_pos_12++) {
      if (OrderSelect(l_pos_12, SELECT_BY_POS, MODE_TRADES)) {
         if (OrderSymbol() == Symbol() && OrderMagicNumber() == 13938)
...
void HistoryClosePositionToJurnal(double BeginBalance = 10000);
...
void Histroy_TradesToDll_Params(string SignalBaseName, string Params);
//---записва сключените сделки в файл
void HistoryClosePositionToFile(bool AppendFile = false, double BeginBalance = 10000, string FileName = "history.txt");
//---пресмята математичното очакване //PipsCalc - задава дали да работи с пипсове (true) или с долари (false)// 
double MOSystem(bool PipsCalc = true);
//---пресмята печалбата и възвращаемостта по години
double AverageAnnualReturn(bool beginbalance = 10000);
...
void DropDownToFile(bool AppendFile = false, double BeginBalance = 10000, string FileName = "dropdown.txt");
//Записва в файл печалбата по месеци;
void SumMonthlyProfitToFile(string FileName = "monthly_profit.txt");
...
{ 
   if (IsTesting()){         
      if (!IsOptimization()){
         HistoryClosePositionToFile(false, 1000, "history_OldSys3938.txt");
         DropDownToFile(false, 1000, "dropdown_OldSys3938.txt");
         AverageAnnualReturn(1000);
         SumMonthlyProfitToFile("monthly_profit_OldSys3938.txt");
         MOSystem(true);                          
      }                      
      Histroy_TradesToDll("OldSys3938");      
   }             
   return(0);
}
