...
void Bias_Process(){
   ...
   if (GetBias_Direction() == 1){
      ...
      if (close < GetBias_Level()){            
         SetBiasToBuff(shift, GetBias_Level());  //поствя точка на нивото на пробива - ползва се само за проверка дали работи корекно
         Bias_BreakProcess(); 
      }else{         
         Bias_Continue();                   
      }         
...
int Bias_CalcHighIndex(int count, int begin){   
   ...
   while (i >= begin-count && !find){
      ...
      if (MLBarHigh(i) > level){
         ...
         if (bias_bar_count == BiasBarCount) find = true;                       
      } 
      i--;      
