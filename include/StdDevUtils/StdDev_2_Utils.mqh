...
int PreparePendingOrder(int MAGIC, string SignalName, int TradeType, datetime SignalTime, 
   ...
   if (TradeType == OP_BUYSTOP){                           
      ...
      //za fiksiran stop
      ldStop = CalculateStopLoss(TradeType, SL_TIMEFRAME, StopLoss_pips, 0, ldLevel, MaxStopLoss_pips);
      //ldStop = CalculateStopLoss(TradeType, SL_TIMEFRAME, SL_StdDevPeriod, SL_StdDevRatio, ldLevel, MaxStopLoss_pips);
      Print("ldStop = ", ldStop);
            
               
      ldLot = CalcLotSizeMM(MAGIC, NormalizeDouble((ldLevel - ldStop)/Point, 0), CurrCountOrders);                  
      //comment = comment + "#" + DoubleToStr(MathAbs((ldLevel - ldStop)/Point),0) + "#"+ DoubleToStr(Period(), 0);      
      comment = OrderComment_Build(0, SignalName, SignalTime, MathAbs((ldLevel - ldStop)/Point));

      if (Limit_pips > 0) ldLimit = ldLevel + Limit_pips*Point;
      //Корекция на целта спрямо лимитите на брокера
      if (ldLimit > 0) ldLimit = MathMax(ldLimit, ldLevel + MinBrokerLevel()*Point);   
   ...
   if (TradeType == OP_SELLSTOP){ 
      ...
      ldLot = CalcLotSizeMM(MAGIC, NormalizeDouble((ldStop - ldLevel)/Point, 0), CurrCountOrders);   
      //comment = comment + "#" + DoubleToStr(MathAbs((ldStop - ldLevel)/Point),0) + "#" + DoubleToStr(Period(), 0);      
      comment = OrderComment_Build(0, SignalName, SignalTime, MathAbs((ldStop - ldLevel)/Point));
   
      if(Limit_pips > 0) ldLimit = ldLevel - Limit_pips * Point;   
      //Корекция на целта спрямо лимитите на брокера
      if(ldLimit > 0) ldLimit = MathMin(ldLimit, ldLevel - MinBrokerLevel()*Point);                         
...
double CalculateStopLoss(int TradeType, int TimeFrame, double StopParam1, double StopParam2, double SignalLevel, int MaxStopLoss = 0){
   if (StopParam1 == 0 && StopParam2 == 0 && MaxStopLoss == 0) return(0);
   ...
   if (TradeType == OP_SELL  || TradeType == OP_SELLSTOP) Result = PriceLevel + dev + Ask - Bid + 1*Point;      
...
//------------------     
   ...
   if(Result > 0 && MaxStopLoss > 0){       
      if (TradeType == OP_BUY || TradeType == OP_BUYSTOP) Result = MathMax(Result, PriceLevel - MaxStopLoss*Point) - 1*Point; 
      if (TradeType == OP_SELL || TradeType == OP_SELLSTOP) Result = MathMin(Result, PriceLevel + MaxStopLoss*Point) + Ask - Bid + 1*Point;         
   }   
...
int SAV_TimeFrameFromString( string value )
{
...
//---------------------------------------------------------------
//-------------------------------------------------------------------------------------------
//"име на сигнала # Time на фрактала # начален стоп # Времеви период на сигнала "
//"s8fr#874358321#33#1440"

string InternalSignalName(string comment){
   return(OrderComment_GetName(comment));
/*
