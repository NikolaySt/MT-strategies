...
{ 
SM1=iStochastic(NULL,0,Kperiod,Dperiod,Slowing,method,1,MODE_MAIN,1);
SS1=iStochastic(NULL,0,Kperiod,Dperiod,Slowing,method,1,MODE_SIGNAL,1);
SM2=iStochastic(NULL,0,Kperiod,Dperiod,Slowing,method,1,MODE_MAIN,2);
SS2=iStochastic(NULL,0,Kperiod,Dperiod,Slowing,method,1,MODE_SIGNAL,2);
ADX1=iADX(NULL,0,ADXperiod,0,MODE_MAIN,1);
ADX2=iADX(NULL,0,ADXperiod,0,MODE_MAIN,2);
DPLUS=iADX(NULL,0,ADXperiod,0,MODE_PLUSDI,1);
DMINUS=iADX(NULL,0,ADXperiod,0,MODE_MINUSDI,1);
MA=iMA(NULL,0,MAperiod,0,MODE_SMMA,0,1);
...
if (Rules()==1 && ScanTradesOpen(Magic)==0 && UseMA)
   ...
   if (!IsOptimization())
 {
         ...
         ObjectSet   ("BB"+TimeCurrent(), OBJPROP_PRICE1,iLow(0,0,1));   
 }
TakeProfit=TakeMA*Dec*Point;StopLoss=StopMA*Dec*Point;
   ...
   if (!IsOptimization())
 {
         ...
         ObjectSet   ("BB"+TimeCurrent(), OBJPROP_TIME1,TimeCurrent()-Period());
         ObjectSet   ("BB"+TimeCurrent(), OBJPROP_PRICE1,iHigh(0,0,1)+Period()/10*Point);   
 }
TakeProfit=TakeMA*Dec*Point;StopLoss=StopMA*Dec*Point;
...
if (ADX1>ADXLevel && ADX1>ADX2 && DPLUS>DMINUS && iClose(0,0,1)>MA) return(1);
if (ADX1>ADXLevel && ADX1>ADX2 && DPLUS<DMINUS && iClose(0,0,1)<MA) return(2);
...
void CloseByMA(int Magic)
{
    ...
    { 
      OrderSelect(cnt, SELECT_BY_POS,MODE_TRADES);     
        ...
        if (OrderMagicNumber()==Magic && OrderType()==OP_BUY && iClose(0,0,1)<MA)     
        {
 ...
 for (try=1;try<=MaxTries;try++)
       {		 
        ...
        if (OrderMagicNumber()==Magic && OrderType()==OP_SELL && iClose(0,0,1)>MA)     
        {
