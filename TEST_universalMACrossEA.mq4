...
extern int              MinCrossDistance     = 0,    //Always positive, 0:disable
                        MaxCrossDistance     = 0,    //Always positive, 0:disable
...
//----------------------- SET MinCrossDistance ALWAYS POSITIVE
   ...
   MaxCrossDistance = MathAbs(MaxCrossDistance);
...
//----------------------- SET VALUE FOR VARIABLE
   if(ConfirmedOnEntry==true)
   {
      ...
      FastMACurrent    = 
         //iCustom(NULL, TimeFrame, "SAN_Bias", FastMAPeriod, 0, 1);
         iMA(NULL,TimeFrame,FastMAPeriod,FastMAshift,FastMAType,FastMAPrice,1);
      SlowMACurrent    = 
         iCustom(NULL, TimeFrame, "Ard_BiasLogicExp", SlowMAPeriod, 150, 0, 1);
   ...
   else
   {
      FastMACurrent    = 
         //iCustom(NULL, TimeFrame, "SAN_Bias", FastMAPeriod, 0, 1);
         iMA(NULL,TimeFrame,FastMAPeriod,FastMAshift,FastMAType,FastMAPrice,0);
      SlowMACurrent    = 
         iCustom(NULL, TimeFrame, "Ard_BiasLogicExp", SlowMAPeriod, 150, 0, 1);
         //iMA(NULL,TimeFrame,SlowMAPeriod,SlowMAshift,SlowMAType,SlowMAPrice,0);
   }
...
//+------------------------------------------------------------------+
   if(MaxLookUp>0 && (MinCrossDistance > 0 || MaxCrossDistance > 0) )
   {
      BuyCondition  = false;
      SellCondition = false;
      ...
      if(shiftCROSS <= MaxLookUp && Distance >= MinCrossDistance && Distance <= MaxCrossDistance)
      {
         if(ReverseCondition==false)
         {
      ...
      if(PrintControl==true)
      {
         Print(TimeToStr(CrossTime,TIME_MINUTES)," - ",shiftCROSS," - ",Distance," - min=",MinCrossDistance, ", max=", MinCrossDistance, " - ",TradeDirection);
...
//+------------------------------------------------------------------+
   ...
   {
      ThirdMAValue = 
         iCustom(NULL, TimeFrame, "Ard_BiasLogicExp", ThirdMAPeriod, 150, 0, 1);
         //iMA(NULL,TimeFrame,ThirdMAPeriod,ThirdMAshift,ThirdMAType,ThirdMAPrice,0);
