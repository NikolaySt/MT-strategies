...
int start(){
   ...
   for (int i = limit; i >= 1; i--) {
      ...
      /*
      if (IsVisualMode() && Time[i] >= StrToTime("2006.09.06 00:00")){
         Print(
            ", trend=",trend,
            ", Time=",TimeToStr(Time[i]),
            ", i=",i,
            ", close=",close,
            ", iChimoku_GetTenkanSen(i)=",iChimoku_GetTenkanSen(i),
            ", iChimoku_GetKijunSen(i)=", iChimoku_GetKijunSen(i),
            ", MinMaxByDir(Cloude(i))=",MinMaxByDir(1, iChimoku_GetSenkouSpanA(i+KijinSen), iChimoku_GetSenkouSpanB(i+KijinSen)),
            ", iChimoku_GetChinkouSpan(i+KijinSen)=",iChimoku_GetChinkouSpan(i+KijinSen),
            ", low=",low,
            ", high=",high
         );  
      }   
